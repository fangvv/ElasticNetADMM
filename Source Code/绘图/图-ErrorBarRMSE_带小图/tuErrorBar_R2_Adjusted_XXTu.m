...
legend('ADMM分布式算法','Independent分布式算法','location','SouthWest');
set(gcf,'Units','centimeters','Position',[6 6 14.5 12]);
set(gca,'Position',[.15 .15 .8 .75]);
set(get(gca,'XLabel'),'FontSize',16);
...
set(gca,'XTickMode','manual','XTick',[1 20 40 60],'YTick',[0.96 0.97 0.98 0.99 1]); 
set(gca,'FontSize',12);
...
% %% 箭头
annotation('textarrow',[.6,.6],[.8,.5],'linewidth',1.5);
% %% 再次生成子图
axes('Position',[0.25,0.20,0.40,0.23]);   
% %% 子数据集
xa = 1:3;
testR2Adjusted0 = testR2Adjusted(xa,:);
CentralizedtestR2Adjusted_mid0 = CentralizedtestR2Adjusted_mid(xa,:);
...
% 
plot(xAxis0,testR2Adjusted0,'s--','linewidth',1.5);
set(gca,'XTickMode','manual','XTick',[1 20 40],'YTick',[0.996 0.997 0.998 0.999 1]); 
set(gca,'FontSize',12);
grid on;set(gca,'GridLineStyle',':','GridColor','k','GridAlpha',1);
axis([-5,45,0.996,1]);
hold on;
%errorbar(xAxis0,CentralizedtestR2Adjusted_mid0,volume_std0,'>:','linewidth',1.5);
plot(xAxis0,CentralizedtestR2Adjusted_mid0,'>:','linewidth',1.5);
%% 保存图像
